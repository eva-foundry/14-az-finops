...
<#$
.SYNOPSIS
Configure EsDAICoESub subscription cost export to FinOps Hub storage.

.DESCRIPTION
Creates or updates a subscription-level Cost Management export for EsDAICoESub
...
This script assumes:
- You have Reader + Cost Management Contributor on EsDAICoESub.
...
- A subscription Owner/Contributor has already registered
  Microsoft.CostManagementExports for EsDAICoESub.

It is safe to run multiple times; the export will be created or updated
idempotently.

.PARAMETER SubscriptionId
Target subscription ID (default: EsDAICoESub).
...
./Configure-EsDAICoESub-CostExport.ps1 -TriggerImmediately

.EXAMPLE
# Configure export but let the scheduled run execute at midnight UTC
./Configure-EsDAICoESub-CostExport.ps1
#>

param(
    [string]$SubscriptionId = "d2d4e571-e0f2-4f6c-901a-f88f7669bcba",  # EsDAICoESub
    [string]$ExportName = "EsDAICoESub-Daily",
    [string]$StorageResourceGroup = "EsDAICoE-Sandbox",
    [string]$StorageAccount = "marcosandboxfinopshub",
    [string]$ContainerName = "costs",
    [string]$RootFolderPath = "EsDAICoESub",
...
Write-Host "EsDAICoESub Cost Export Configuration" -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
...
# Check Microsoft.CostManagementExports registration state
$providerState = $null
try {
    $providerState = az provider show --namespace Microsoft.CostManagementExports --query "registrationState" -o tsv 2>$null
}
catch {
    $providerState = $null
}
...
if ($providerState -ne "Registered") {
    Write-Host "  [ERROR] Resource provider Microsoft.CostManagementExports is not registered for this tenant/subscription." -ForegroundColor Red
    Write-Host "" 
    Write-Host "  Ask a subscription Owner or Contributor on EsDAICoESub to run:" -ForegroundColor Yellow
    Write-Host "    az account set --subscription $SubscriptionId" -ForegroundColor Gray
    Write-Host "    az provider register --namespace Microsoft.CostManagementExports" -ForegroundColor Gray
    Write-Host "" 
    Write-Host "  After the provider shows as Registered, rerun this script." -ForegroundColor Yellow
    exit 1
}

Write-Host "  [PASS] Microsoft.CostManagementExports is registered" -ForegroundColor Green
Write-Host "" 
...
try {
    ...
    Write-Host "" 
    Write-Host "[SUCCESS] EsDAICoESub export configured" -ForegroundColor Green
